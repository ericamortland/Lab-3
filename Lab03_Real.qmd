...
author:
- name: "Erica Mortland"
  url: "https://ericamortland.github.io/"
...
Open environmental and health data is power beacsue when it is readily accessible, it enables scientists to establish climate trends over decades which makes it possible to create predictive models. It also provides the evidence necessary for effective resource management and conservation efforts. 
Additionally, when this open data becomes inaccessible long-term trend analysis becomes difficult as well as researchers losing the ability to reproduce and create findings.
...
county_population <- tibble(
  county = c("Adams", "Arapahoe", "Denver", "El Paso", "Jefferson", "Boulder", "Larimer", "Weld", "Douglas", "Pueblo"),
  population = c(519572, 655070, 715522, 730395, 582910, 330758, 359066, 328981, 357978, 168162)
)
```
...
After  filtering the population data to only include Colorado counties, the dataset contained 64 rows and 3 columns. 
The columns are: 
...
The filtered dataset is good for joining with the Colorado COVID data using the FIPS column, which we can normalize COVID case counts based on population size with it.

```{r}
range(population_data$Population_2021)
```

The population of Colorado counties in 2021 ranges from 741 people to 737,287 people in the largest county. This wide range makes it important to normalize COVID case counts by population, as raw case counts alone would unfairly compare large and small counties.
...
Adjusting for population size played a crucial role in the analysis by allowing for a more balanced comparison between states of varying sizes. Without this adjustment, states with larger populations, such as California and New York, would naturally report higher case counts, making it difficult to compare them fairly to smaller states like Ohio. By normalizing the data based on population, the focus shifts from raw case numbers to infection rates, providing a clearer picture of how widespread the virus is in each state. This approach emphasizes the importance of assessing prevalence relative to population rather than relying solely on total case counts.
...
The graph illustrates how the weighted mean center of COVID-19 cases changed over time. The visualization reveals a distinct west-to-east shift across the country, with points color-coded by month and scaled according to total case numbers. A noticeable curve emerges, tracing the movement of the weighted center of cases through the southern United States. At the onset of the pandemic, cases were primarily concentrated in the western and central regions, but as time went on, the focal point of infections gradually shifted toward the eastern and southeastern parts of the country.
